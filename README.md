...
###### Pesquisa pela sequência na saída do comando (substituir comando e sequência de acordo com os valores a serem
###### buscados).
    locate arq
...
### Cria um pacote TAR (nomeado pacote.tar) com os arquivos especificados (substituir a variável arqs pelo nome do
### arquivo).
    tar xf pacote.tar
...
### Retorna informações de DNS para o domínio (substituir a variável host pelo domínio de um site ou o número IP).
    9ListAllCommands | grep searchstr
...
### Instala um pacote DEB (distros Debian) (substituir a variável pacote.deb pelo nome do pacote de programa).
    rpm -Uvh pacote.rpm
### Instala um pacote RPM (Distros que utilizam RPM) (substituir a variável pacote.rpm pelo nome do pacote de
programa).
...
### Exibe o tamanho dos arquivos e diretórios ordenados por tamanho.
    du -sk * | sort -rn
### Exibe o espaço usado por pacotes .rpm instalados e organizados pelo tamanho (Fedora, Red Hat e
### outros).
...
### Cria uma nova forma de arquivo de entrada assumindo que está codificado em fromEncoding e converte
### para ToEncoding.
    find . -maxdepth 1 -name *.jpg -print -exec convert ”{}” -resize 80×60 “thumbs/{}” \;
### Agrupa arquivos dimensionados no diretório atual e os envia aos diretórios de visualização em miniaturas
#### (requer o conversor do ImagemagicK).
### 4.4.
#### Encontrar arquivos
    find / -name file1
...
### Monta um disco chamado hda2. Primeiro, verifique a existência do diretório ‘/ mnt/hda2’; Se você não
### possuir, você deve criá-lo.
    umount /dev/hda2
...
### Monta um cdrom / dvdrom.
    15mount /dev/hdc /mnt/cdrecorder
...
### Exibe (no Debian ou derivados) uma lista com 25 pacotes instalados que consomem mais espaço (em
### ordem decrescente)
### 4.7.
### Usuários e grupos
...
### Define um limite de tempo para a senha do usuário. Neste caso, ele diz que a chave expira a 22 de junho
### de 2016.
### Informações do sistema Linux
##### • pwck
...
### 4.8.
### Permissões de arquivos (+ = Adiciona e - = Remove permissões)
    ls -lh
### exibe permissões.
    ls /tmp | pr -T5 -W$COLUMNS
### Divide o terminal em 5 colunas.
    chmod ugo+rwx directory1
### Define permissões de leitura®, gravar (w) e executar (x) para o dono (u), grupo (g) e outros (ou) no
### diretório ‘arquivo1’.
...
### Ver todos os arquivos com sistema SUID configurado.
    chmod u+s /bin/file1
### Define o bit SUID em um arquivo binário. O usuário que está executando esse arquivo adquire os mesmos
### privilégios como proprietário.
    chmod u-s /bin/file1
...
### Conjunto STIKY bit em um diretório. Permite a exclusão de arquivos somente para os legítimos
### proprietários.
    chmod o-t /home/public
### Desativa STIKY bit em um diretório.
### 4.9.
### Atributos especiais de arquivo: (+ = Adiciona e - = Remove permissões)
    chattr +a file1
...
### Extrai um tarball em / tmp.
    19tar -cvfj archive.tar.bz2 dir1
...
### 214.12. Pacotes YUM Updater (Red Hat, Fedora e derivados)
...
### 244.17. Estabelecer o formato de conversão de arquivos
...
### Salva os diretórios/etc e a raiz (excluindo o conteúdo do subdiretório/root/dir1 /) em um arquivo
### compactado, cujo nome inclui a data e hora atual.
     26( cd /tmp/local/ && tar c . ) | ssh -C user@ip_addr ‘cd /home/share/ && tar x -p’
...
### Configura uma PAT (conversão de endereços de porta) na eth0, escondendo os pacotes de saída de
### coação.
